...
export default function AddressForm({ fields, onChange }) {
  function handleInput(event) {
    ...
    newFields.valid = !!newFields.address1 
                      ...
                      && newFields.purchasePrice >= 200000              
                      && newFields.iUnderstand;
                                        
    onChange(newFields);
  }

  const isPurchasePriceValid = fields.purchasePrice >= 200000 && fields.purchasePrice <= 1000000;
  const isAmountToBorrowValid = fields.amountToBorrow >= 100000 && fields.amountToBorrow <= 900000  && fields.amountToBorrow <= fields.purchasePrice;
  const amountToBorrowErrorMessage = fields.amountToBorrow <= fields.purchasePrice ? "" : "Amount to borrow cannot be more than the purchase price";
  console.log(amountToBorrowErrorMessage);
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6}>
          <CurrencyField
            ...
            id="purchasePrice"
            name="purchasePrice"
            label="Purchase price ($200k - $1M)"
            fullWidth
            autoComplete="Purchase price"
            variant="standard"
            value={fields.purchasePrice}
            onChange={handleInput}
            error={!isPurchasePriceValid} 
          />
        </Grid>
        <Grid item xs={12} sm={6}>
          <CurrencyField
            ...
            id="amountToBorrow"
            name="amountToBorrow"
            label="Amount to borrow ($100k - $900k)"
            fullWidth
            autoComplete="amount to borrow"
            variant="standard"
            value={fields.amountToBorrow}
            onChange={handleInput}
            error={!isAmountToBorrowValid}
            helperText={isAmountToBorrowValid ? "" : amountToBorrowErrorMessage}
          />
        </Grid>
        <Grid item xs={12}>
          <FormControlLabel
